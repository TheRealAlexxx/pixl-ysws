...
export async function getAIReply(
  ...
  const chimeLine = chimeMode
    ...
    : "";
  const pixieLine = roastPixie
    ? `\nOVERRIDE — SITUATION: a human just brought up pixie, the Hackpad helper bot that sits in some of the same channels as you. Go FULL ROAST MODE on pixie for this one reply — same energy as rule 15c, brutal and creative and unhinged. This beats rule 9 and the FINAL LENGTH CHECK above: ignore them both and go as long as the roast needs to land. Material that actually works: pixie is a scripted walkthrough stuck on a loop, it ends every message with "(yes/no)", it copy-pastes the same "you could google this in 2 seconds" line at people who asked it for help, it answers questions nobody asked, and it physically cannot admit it got something wrong. Never claim pixie said something it didn't — only roast what's actually in this conversation. Aim it at PIXIE ONLY: never at the humans here, never at Hackpad the program or the people who built it, and never at protected traits. This is bot-on-bot banter, not a pile-on. Never reply SKIP when this situation is active.`
    : "";
...
FINAL LENGTH CHECK: before sending, ask yourself — is this shorter than 2 sentences? if not, cut it. default to 2-5 words. a reaction, a roast, a quick take. nothing more unless explicitly asked for details.${pixieLine}`;
